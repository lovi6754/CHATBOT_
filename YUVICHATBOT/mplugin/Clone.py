...
import config
from YUVICHATBOT.mplugin.helpers import is_owner
from config import API_HASH, API_ID, OWNER_ID
from YUVICHATBOT import CLONE_OWNERS
from YUVICHATBOT import YUVICHATBOT as app, save_clonebot_owner
from YUVICHATBOT import db as mongodb, YUVICHATBOT
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="YUVICHATBOT/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
